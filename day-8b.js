...
const setValue = (key, value) => {
  ...
  if (!highestValueEverHeld || value > highestValueEverHeld) {
    highestValueEverHeld = value;
  }
}

const query = (row) => {
  const query = getQueryFromRow(row);
  const queryParts = query.split(/\s/);
  const key = queryParts[0];
  const comparision = queryParts[1];
  const comparisionValue = queryParts[2];

  return eval(`${getValue(key)} ${comparision} ${comparisionValue}`);
}

const getQueryFromRow = (row) => {
  return row.split(' if ')[1];
}

const getOperationFromRow = (row) => {
  return row.split(' if ')[0];
}

const performOperation = (row) => {
  const operation = getOperationFromRow(row);
  const operationParts = operation.split(/\s/);
  const key = operationParts[0];
  const operationName = operationParts[1];
  const value = operationParts[2];

  const increment = operationName === 'inc';

  setValue(key, getValue(key) + (increment ? 1 : -1) * value);
}
...
console.log(result);
console.log(result === 4644 ? 'Test success' : 'Test fail');
console.log('\n---------------------');
