const solvePuzzle = (input) => {
  ...
  setValueOfChildren(root);

  const weirdRow = findDiscrepancy(root);

  return findCorrectValueForWeirdChild(weirdRow);
}
...
const parseRows = (rows) => {
  ...
  for (let row of rows) {
    ...
    const value = parseInt(row.split('(')[1].split(')')[0]);
    const children = (row.split(' -> ')[1] || '').split(/,|\s/).filter(item => item);
    parsedRows.push({
      name,
      value,
      children
...
const setChildren = (rows) => {
  for (let row of rows) {
    if (row.children.length === 0 || typeof row.children[0] === 'object') continue;

    const children = rows.filter(item => row.children.indexOf(item.name) > -1);
...
const findRoot = (rows) => {
  for (let row of rows) {
    const parent = rows.filter(item => item.children.indexOf(row) > -1)[0];
    if (!parent) {
      return row;
    }
  }
...
const setValueOfChildren = (row) => {
  row.children.forEach(setValueOfChildren);

  row.childrenValues = row.children.reduce((acc, child) => acc + child.totalValue, 0);
  row.totalValue = row.value + row.childrenValues;
}

const findDiscrepancy = (row) => {
  let childTotalValue;
  for (let child of row.children) {

    const childDiscrepancy = findDiscrepancy(child);
    if (childDiscrepancy) return childDiscrepancy;

    if (!childTotalValue) {
      childTotalValue = child.totalValue;
      continue;
    }

    if (child.totalValue !== childTotalValue) {
      return row;
    }
  }
}

const findCorrectValueForWeirdChild = (row) => {
  const childrenValues = {};
  for (child of row.children) {
    childrenValues[child.totalValue] = childrenValues[child.totalValue] ? childrenValues[child.totalValue] + 1 : 1;
  }
  
  const weirdChildValue = parseInt(Object.keys(childrenValues).filter(key => childrenValues[key] === 1)[0]);
  // Below would only work if there's more than two children when one is wrong
  const correctChildValue = parseInt(Object.keys(childrenValues).filter(key => childrenValues[key] !== 1)[0]);
  
  const weirdChild = row.children.filter(item => item.totalValue === weirdChildValue)[0];

  const diff = correctChildValue - weirdChildValue;
  return weirdChild.value + diff;
}
...
console.log(result);
console.log(result === 1219 ? 'Test success' : 'Test fail');
console.log('\n---------------------');
