...
const solvePuzzle = (input) => {
  ...
  while(true) {
    ...
    const matchingPreviousResultIndex = findMatchingPreviousResult(banks);
    if (matchingPreviousResultIndex !== -1) {
      return Math.abs(previousResults.length - matchingPreviousResultIndex);
    };
...
const findMatchingPreviousResult = (banks) => {
  ...
  const previousResult = previousResults.filter((prevBanks) => {
    prevBanksString = prevBanks.join(',');
    return banksString === prevBanksString;
  })[0];

  return previousResults.indexOf(previousResult);
}
...
console.log(result);
console.log(result === 2765 ? 'Test success' : 'Test fail');
console.log('\n---------------------');
