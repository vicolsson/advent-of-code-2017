const solvePuzzle = (input) => {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (let w = 0; w < words.length && !foundError; w++) {
      for (let w2 = 0; w2 < words.length && !foundError; w2++) {
        if (w === w2) continue;
        ...
        if (words[w] === words[w2]) {
          ...
          const wLetters = words[w].split('').sort().join('');
          const w2Letters = words[w2].split('').sort().join('');
          if (wLetters === w2Letters) {
            foundError = true;
          }
...
console.log(result);
console.log(result === 186 ? 'Test success' : 'Test fail');
console.log('\n---------------------');
