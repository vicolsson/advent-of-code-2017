const solvePuzzle = (input) => {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const numbers = getEvenlyDivisibleOfRow(row);

    const divisionRes = numbers[0] / numbers[1];
    sum += divisionRes;
  }
...
const getEvenlyDivisibleOfRow = (row) => {
  ...
  throw 'Evenly divisible numbers not found';
};
...
console.log(result);
console.log(result === 214 ? 'Test success' : 'Test fail');
console.log('\n---------------------');
